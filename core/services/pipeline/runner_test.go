...
`
	...
	t.Run("test min non-bootstrap", func(t *testing.T) {
		...
		s := fmt.Sprintf(minimalNonBootstrapTemplate, cltest.NewEIP55Address(), ek.PeerID, transmitterAddress, kb.ID, "http://blah.com", "")
		_, err = services.ValidatedOracleSpecToml(config.Config, s)
		require.NoError(t, err)
		err = toml.Unmarshal([]byte(s), &os)
		require.NoError(t, err)
	...
	t.Run("test job spec error is created", func(t *testing.T) {
		...
		kb, _, err := keyStore.GenerateEncryptedOCRKeyBundle()
		require.NoError(t, err)
		spec := fmt.Sprintf(ocrJobSpecTemplate, cltest.NewAddress().Hex(), ek.PeerID, kb.ID, transmitterAddress, fmt.Sprintf(simpleFetchDataSourceTemplate, "blah", true))
